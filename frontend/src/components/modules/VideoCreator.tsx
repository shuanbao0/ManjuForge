...
export default function VideoCreator({ onTaskCreated, remixData, onRemixClear, params, onParamsChange }: VideoCreatorProps) {
    ...
    const handlePolish = async (feedback: string = "") => {
        ...
        try {
            ...
            alert(t("modules.video.polishFailed"));
        } finally {
    ...
    const handleReferenceVideoSelect = (videoUrl: string) => {
        if (selectedReferenceVideos.includes(videoUrl)) {
            ...
            if (selectedReferenceVideos.length >= 3) {
                alert(t("modules.video.maxReferenceVideos", undefined, "最多选择 3 个参考视频"));
                return;
            }
    ...
    const handleSubmit = async () => {
        ...
        if (generationMode === 'i2v') {
            ...
            if (filledSlots.length === 0) {
                alert(t("modules.video.r2vNeedSlot", undefined, "R2V 模式请至少填充一个角色槽位 (@Ref_A)"));
                return;
            }
        ...
        try {
            ...
            alert(t("modules.video.submitFailed"));
                ...
                <div className="flex flex-col gap-6 max-w-4xl mx-auto w-full pb-8">
                    ...
                    <div className="flex items-center justify-center">
                        <div className="flex bg-black/40 rounded-xl p-1.5 gap-1 border border-white/10">
                            ...
                            >
                                <ImageIcon size={16} />
                                {t("modules.video.modeI2VLong", undefined, "🖼️ 首帧驱动 (I2V)")}
                            </button>
                            <button
                                onClick={() => {
                            ...
                            >
                                <Film size={16} />
                                {t("modules.video.modeR2VLong", undefined, "🎬 角色驱动 (R2V)")}
                            </button>
                        </div>
                    </div>
                    ...
                    {generationMode === 'i2v' && (
                        <div className="space-y-4">
                            <div className="flex items-center justify-between">
                                <label className="text-sm font-medium text-gray-300">{t("modules.video.firstFrameLabel", undefined, "首帧图片 (First Frame)")}</label>
                                <div className="flex bg-white/5 rounded-lg p-1 gap-1">
                                    ...
                                    >
                                        <Layout size={14} /> {t("modules.video.tabStoryboard", undefined, "Storyboard")}
                                    </button>
                                    <button
                                    ...
                                    >
                                        <Upload size={14} /> {t("modules.video.tabUpload", undefined, "Upload")}
                                    </button>
                                </div>
                            </div>
                                    ...
                                    <div className="space-y-4">
                                        {currentProject?.frames && currentProject.frames.length > 0 ? (() => {
                                            const completedVideoIds = new Set(
                                                currentProject.video_tasks
                                                    ?.filter((tt: any) => tt.status === "completed")
                                                    .map((tt: any) => tt.id) ?? []
                                            );
                                            return (
                                                    ...
                                                    >
                                                        {(frame.rendered_image_url || frame.image_url) ? (
                                                            ...
                                                            <div className="w-full h-full bg-white/5 flex items-center justify-center text-xs text-gray-500">
                                                                {t("modules.storyboard.noImage", undefined, "No Image")}
                                                            </div>
                                                        )}
                                                        <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                                                            <span className="text-xs text-white font-bold">{t("modules.video.select", undefined, "Select")}</span>
                                                        </div>
                                                        {/* Frame Number Badge */}
                                                        ...
                                                        {prevVideoCompleted && (
                                                            <button
                                                                ...
                                                                title={hasExtracted ? t("modules.video.reExtractPrevEnd", undefined, "Re-extract previous video's last frame") : t("modules.video.usePrevEndAsInput", undefined, "Use previous video's last frame as input")}
                                                            >
                                                                {isExtracting ? (
                                                                    <Loader2 size={10} className="animate-spin" />
                                                                ) : hasExtracted ? (
                                                                    <><Check size={10} /> {t("modules.video.applied", undefined, "Applied")}</>
                                                                ) : (
                                                                    <><Film size={10} /> {t("modules.video.prevEndFrame", undefined, "Prev End Frame")}</>
                                                                )}
                                                            </button>
                                                        )}
                                                    </div>
                                            ...
                                            <div className="flex flex-col items-center justify-center h-[200px] text-gray-500 gap-2">
                                                ...
                                                <p className="text-xs">{t("modules.video.noStoryboardFrames", undefined, "No storyboard frames found.")}</p>
                                            </div>
                                        )}
                                        ...
                                        {selectedImages.length > 0 && (
                                            <div className="pt-4 border-t border-white/10">
                                                <p className="text-xs text-gray-500 mb-2">{t("modules.video.selectedForGeneration", undefined, "Selected for Generation:")}</p>
                                    ...
                                    <div className="space-y-4">
                                        <div className="grid grid-cols-3 gap-4">
                                            ...
                                            >
                                                ...
                                                <p className="text-gray-400 text-xs font-medium">{t("modules.video.addImage", undefined, "Add Image")}</p>
                                            </div>
                                        </div>
                                        ...
                                        {availableAssets.length > 0 && (
                                            <div className="mt-4 pt-4 border-t border-white/10">
                                                <p className="text-xs text-gray-500 mb-2">{t("modules.video.quickSelectAssets", undefined, "Quick Select from Assets:")}</p>
                    ...
                    {generationMode === 'r2v' && (
                        <div className="space-y-6">
                            ...
                            <div className="space-y-3">
                                <label className="text-sm font-medium text-gray-300">{t("modules.video.selectFrame", undefined, "选择分镜 (Select Frame)")}</label>
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-[200px] overflow-y-auto custom-scrollbar pr-2">
                                    {currentProject?.frames && currentProject.frames.length > 0 ? (
                                                ...
                                                <div className="flex items-start gap-3">
                                                    ...
                                                    <div className="flex-1 min-w-0">
                                                        ...
                                                        <p className="text-xs text-gray-300 line-clamp-2">
                                                            {frame.action_description || frame.image_prompt || t("modules.video.noDescription", undefined, "暂无描述")}
                                                        </p>
                                                        {frame.dialogue && (
                                        ...
                                        <div className="col-span-2 flex flex-col items-center justify-center h-[100px] text-gray-500 gap-2">
                                            ...
                                            <p className="text-xs">{t("modules.video.noFramesGoStoryboard", undefined, "无分镜数据，请先在 Storyboard 阶段生成分镜")}</p>
                                        </div>
                                    )}
                                </div>
                            </div>
                            ...
                            {/* Cast Slots (卡司槽位) */}
                            <div className="space-y-3">
                                <label className="text-sm font-medium text-gray-300">{t("modules.video.castSlots", undefined, "卡司槽位 (Cast Slots)")}</label>
                                <div className="grid grid-cols-3 gap-4">
                                    {[0, 1, 2].map((slotIndex) => {
                                        const slot = castSlots[slotIndex];
                                        const slotLabel = `@Ref_${String.fromCharCode(65 + slotIndex)}`; // @Ref_A, @Ref_B, @Ref_C
                                        const slotTitle = slotIndex === 0 ? t("modules.video.slotProtagonist", undefined, "主角") : t("modules.video.slotSupporting", undefined, "配角");
                                            ...
                                            >
                                                ...
                                                <div className="absolute top-2 left-2 z-10">
                                                    <span className="text-[10px] px-2 py-0.5 rounded-full bg-purple-600 text-white font-bold">
                                                        {t("modules.video.characterSlotN", { n: slotIndex + 1 }, `角色${slotIndex + 1}`)}
                                                    </span>
                                                </div>
                                                ...
                                                {slot?.url ? (
                                                    ...
                                                    <div className="aspect-video flex flex-col items-center justify-center p-4">
                                                        ...
                                                        >
                                                            <option value="">{t("modules.video.selectReferenceVideo", undefined, "选择参考视频...")}</option>
                                                            {availableReferenceVideos.map((v, i) => (
                                                                <option key={i} value={v.url}>{v.assetName} - {v.type}</option>
                                                            ))}
                                                        </select>
                                                        {slotIndex === 0 && (
                                                            <p className="text-[10px] text-amber-400 mt-2">{t("common.required")}</p>
                                                        )}
                                                    </div>
                                                )}
                                            </div>
                                ...
                                {availableReferenceVideos.length === 0 && (
                                    <p className="text-xs text-amber-400 bg-amber-500/10 border border-amber-500/30 rounded-lg p-3">
                                        {t("modules.video.noReferenceVideosWarning", undefined, "⚠️ 无可用的参考视频。请先在 Assets 阶段为角色/场景生成 Motion Reference 视频。")}
                                    </p>
                                )}
                            </div>
                        </div>
                    ...
                    <div className="space-y-2">
                        <div className="flex justify-between items-center">
                            <label className="text-sm font-medium text-gray-300">{t("modules.video.promptLabelLong", undefined, "提示词 (Prompt)")}</label>
                            <div className="flex items-center gap-2">
                                {generationMode === 'i2v' && (
                                    <div className="relative">
                                        ...
                                        >
                                            <Video size={12} /> {t("modules.video.cameraMove", undefined, "运镜")}
                                        </button>
                                    </div>
                                )}
                                <button
                                ...
                                >
                                    ...
                                    {t("modules.video.aiPolish", undefined, "AI 润色")}
                                </button>
                                <button
                                    ...
                                    title={t("modules.video.clearPrompt", undefined, "Clear Prompt")}
                                >
                                    <Eraser size={12} /> {t("modules.video.clear", undefined, "清空")}
                                </button>
                            </div>
                        </div>
                                ...
                                {[0, 1, 2].map((idx) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <span>{t("modules.video.insertCharacter", { name: slot?.name || t("modules.video.characterSlotN", { n: idx + 1 }, `角色${idx + 1}`) }, `插入 ${slot?.name || `角色${idx + 1}`}`)}</span>
                                        </button>
                                    );
                                })}
                            </div>
                        ...
                        <div className="relative">
                            <PromptBuilder
                                ...
                                placeholder={generationMode === 'r2v'
                                    ? t("modules.video.promptPlaceholderR2V", undefined, "输入提示词... \n插入角色格式: [character1:名称]\n插入运镜格式: (camera: 运镜指令)")
                                    : t("modules.video.promptPlaceholderI2V", undefined, "输入提示词，描述画面内容...\n插入运镜格式: (camera: 运镜指令)")
                                }
                            />
                        </div>
                        ...
                        <AnimatePresence>
                            {polishedPrompt && (
                                ...
                                >
                                    ...
                                    <div className="space-y-1">
                                        <div className="flex justify-between items-center">
                                            <span className="text-[10px] font-bold text-gray-500 uppercase">{t("modules.video.englishForGeneration", undefined, "English (生成用)")}</span>
            ...
            < div className="p-6 border-t border-white/10 bg-black/40 backdrop-blur-md z-10" >
                <div className="max-w-4xl mx-auto w-full">
                    ...
                    >
                        {isSubmitting ? (
                            <>
                                <Loader2 className="animate-spin" /> {t("modules.video.submitting", undefined, "提交中...")}
                            </>
                        ) : submitSuccess ? (
                            <>
                                <Plus /> {t("modules.video.queued", undefined, "已加入队列")}
                            </>
                        ) : (
                            <>
                                <Plus /> {t("modules.video.addToQueue", undefined, "加入生成队列 (Ctrl+Enter)")}
                            </>
                        )}
                    </button>
                    <div className="flex justify-center mt-3">
                        <label className="flex items-center gap-2 text-xs text-gray-500 cursor-pointer hover:text-gray-400">
                            ...
                            {t("modules.video.clearAfterSubmit", undefined, "提交后清空内容")}
                        </label>
                    </div>
                </div>
            </div >
