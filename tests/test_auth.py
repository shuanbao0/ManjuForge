...
def test_setup_creates_no_model_instances(client: TestClient):
    """Onboarding must NOT auto-create ModelInstances. Every callable model
    is manually configured by the user — the system never picks an SKU on
    their behalf, so there are no surprise defaults pointing at a model the
    ...
    with session_scope() as s:
        assert s.query(ModelInstanceRow).count() == 0
