...
| 系统 | 路径 |
|------|------|
| macOS / Linux | `~/.manju-forge/`（旧版 `~/.lumen-x/` 会在首次启动时自动迁移） |
| Windows | `C:\Users\<用户名>\.manju-forge\`（旧版 `.lumen-x` 自动迁移） |

---

## 🔑 API 密钥配置

ManjuForge 使用阿里云灵积平台(DashScope)提供 AI 能力。
...
| OSS_BUCKET_NAME | Bucket 名称 | `my-manju-forge-bucket` |
| OSS_ENDPOINT | OSS 地域节点 | `oss-cn-beijing.aliyuncs.com` |
| OSS_BASE_PATH | 存储路径前缀 | `manju-forge`（老用户若已用过默认 `lumenx`，请显式设置 `OSS_BASE_PATH=lumenx` 保留旧前缀） |
...
| 系统 | 路径 |
|------|------|
| macOS / Linux | `~/.manju-forge/logs/app.log` |
| Windows | `C:\Users\<用户名>\.manju-forge\logs\app.log` |
...
1. 打开 Finder
2. 按 `Cmd + Shift + G`
3. 输入 `~/.manju-forge/logs` 并回车

**Windows**：
1. 打开资源管理器
2. 在地址栏输入 `%USERPROFILE%\.manju-forge\logs`
3. 按回车
...
A: ManjuForge 使用阿里云灵积平台的 AI 模型进行图像和视频生成。API Key 用于验证您的身份并计费。
...
删除 `~/.manju-forge/` 目录下的 `webview_storage` 文件夹，然后重启应用。
